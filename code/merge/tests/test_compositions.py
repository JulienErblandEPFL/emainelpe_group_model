"""
Tests for the ``merge.methods.dare_uniform`` composition and the
``METHOD_REGISTRY`` dispatch table.

Cross-validation tests are the strongest sanity check that DARE composes
correctly with uniform merging: drop_rate=0.0 + rescale=True must reduce
to the corresponding non-DARE merge.
"""
